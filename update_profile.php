...
if (isset($_POST['submit'])) {
   ...
   if (!empty($email)) {
      ...
      if ($select_email->rowCount() > 0) {
         $message[] = 'Este email ya se ha registrado';
      } else {
   ...
   if (!empty($number)) {
      ...
      if ($select_number->rowCount() > 0) {
         $message[] = 'Este número ya se ha registrado';
      } else {
   ...
   if ($old_pass != $empty_pass) {
      if ($old_pass != $prev_pass) {
         $message[] = '¡La contraseña pasada no coincidente!';
...
<head>
   ...
   <title>Actualizar Perfil | Chiltepin Restaurant</title>
...
<body>
   ...
   <section class="form-container update-form">
      ...
      <form action="" method="post">
         ...
         <input type="password" name="old_pass" placeholder="Introduce tu contraseña actual" class="box" maxlength="50"
            oninput="this.value = this.value.replace(/\s/g, '')">
         <input type="password" name="new_pass" placeholder="Introduce tu nueva contraseña" class="box" maxlength="50"
            oninput="this.value = this.value.replace(/\s/g, '')">
